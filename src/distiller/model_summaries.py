...
def linearcaps2D_visitor(self, input, output, df, model, memo):
    assert isinstance(self, LinearCaps2d)
    if self in memo:
        return

    # Multiply-accumulate operations: MACs = #IFM * #OFM
    # Bias is ignored
    #torch.Size([1, 32, 5, 5, 200, 1, 1, 16, 8]) 
    #torch.Size([64, 32, 5, 5, 1, 1, 1, 8, 1]) 
    # print(self.input_height)
    # print(self.input_width)
    # print(self.output_height)
    # print(self.output_width)
    # print(self.input_caps_types)
    # print(self.input_caps_shape)
    # print(self.output_caps_types)
    # print(self.output_caps_shape)
    ...
    routing_ops = self.output_caps_types*self.output_height*self.output_width*self.output_caps_shape[0]*self.input_caps_types*self.output_caps_shape[1]*self.output_caps_shape[0]*self.output_caps_shape[1]* self.kernel_size[0]*self.kernel_size[1]
    print("routing ops")
    print(routing_ops)
    routing_ops = routing_ops*(self.num_iterations-1)
    weights_vol = macs = routing_ops + self.input_caps_shape[0]*self.input_caps_shape[1]*self.output_caps_shape[0]*self.output_caps_shape[1]*self.input_height*self.input_width*self.input_caps_types*self.output_caps_types
    module_visitor(self, input, output, df, model, weights_vol, macs)

def module_visitor(self, input, output, df, model, weights_vol, macs, attrs=None):
    in_features_shape = input[0].size()
    if isinstance(self, LinearCaps2d):
        output = output[0]
...
def model_performance_summary(model, dummy_input, batch_size=1):
    ...
    def install_perf_collector(m):
        ...
        elif isinstance(m, LinearCaps2d):
            hook_handles.append(
                m.register_forward_hook(
                    partial(linearcaps2D_visitor, df=df, model=model, memo=memo)
                )
            )
